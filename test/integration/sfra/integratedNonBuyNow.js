...
describe('Integrated Non-BuyNow', function () {
    ...
    it('In-Store pickup - simple working case', async function () {
      ...
      await ecUtils.verifyStorePickupAddressInPopup(cpCheckout, ecUtils.store2);
    ...
    it('Express Checkout Finalization Flow. Checks cart checkout and pdp button during finalization flow.', async function () {
      ...
      // Click on confirm button when first installment amount is shown
      let gotItButton = await cpCheckout.waitForSelector('button[automation_id="button-confirmPaymentUpFront"]');
      await gotItButton.click();
      await page.waitForTimeout(1000)
    ...
    it('Test PDP button functionality', async function () {
      ...
      await gotItButton.click();
      // Grab the price
      let totalPrice = await ecUtils.popupGetTotalPrice(cpCheckout);
      ...
      cpCheckout = await newPagePromise;
      await ecUtils.popupLogin(cpCheckout, ecUtils.user.email, ecUtils.user.password);

      // Click on confirm button when first installment amount is shown
      gotItButton = await cpCheckout.waitForSelector('button[automation_id="button-confirmPaymentUpFront"]');
      await gotItButton.click();

      // sfra base defaults to overnight shipping
      totalPrice = await ecUtils.popupGetTotalPrice(cpCheckout);
      expect(totalPrice).to.equal('176.37');
      await cpCheckout.close();
    ...
    it('End-to-end test. Checks amounts in EC popup. Checks shipping method changes. Checks merchant site checkout amounts and addresses.', async function () {
      ...
      await ecUtils.popupClickOnShippingOption(cpCheckout, 2);
