...
var cpBrandUtilities = apUtilities.brandUtilities;
...
var checkoutTools = {
    ...
    getCurrentClearpayPaymentAmount: function(basket) {
        ...
        Logger.debug('basket.getPaymentInstruments : ' + JSON.stringify(basket.getPaymentInstruments()));
        let pi = basket.getPaymentInstruments(paymentMethod);
        Logger.debug('pi.length : ' + pi.length);
        if (pi.length == 0) {
        ...
        Logger.debug('amount : ' + payment.amount);
