...
function Handle(args) {
    ...
    Logger.debug('Token value returned to - AFTERPAY.JS : ' + JSON.stringify(clearPayTokenResponse));
...
function Authorise(args) {
    ...
    var cpInitialStatus = Order.getPaymentInstruments('AFTERPAY')[0].getPaymentTransaction().custom.cpInitialStatus;
    var orderToken = Order.getPaymentInstruments('AFTERPAY')[0].getPaymentTransaction().custom.cpToken;
    ...
    if (ClearpaySession.isValid()) {
        ...
        if (ClearpaySession.getToken() != orderToken) {
            Transaction.begin();
            Order.getPaymentInstruments('AFTERPAY')[0].getPaymentTransaction().custom.cpInitialStatus = cpInitialStatus;
    ...
    if (finalPaymentStatus === 'APPROVED') {
        ...
        return {
            ...
            cpInitialStatus: !empty(Order.getPaymentInstruments('AFTERPAY')[0].getPaymentTransaction().custom.cpInitialStatus) ? Order.getPaymentInstruments('AFTERPAY')[0].getPaymentTransaction().custom.cpInitialStatus : null
        };
