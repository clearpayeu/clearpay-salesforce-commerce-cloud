...
function resetPaymentForms() {
    ...
    var status = Transaction.wrap(function () {
        if (app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value.equals('PayPal')) {
            ...
            cart.removePaymentInstruments(cart.getPaymentInstruments('AFTERPAY'));
            cart.removePaymentInstruments(cart.getPaymentInstruments('CLEARPAY'));
...
function handlePaymentSelection(cart) {
    var clearpayBrandUtilities = ClearpayUtilities.brandUtilities;
    var selectedPaymentMethodID = app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value;
    var result;
    ...
    if ((selectedPaymentMethodID == 'AFTERPAY' || selectedPaymentMethodID == 'CLEARPAY') && !clearpayBrandUtilities.isClearpayApplicable()) {
        result = {
            error: true
        };
    }
    ...
    if (empty(PaymentMgr.getPaymentMethod(selectedPaymentMethodID).paymentProcessor)) {
        result = {
            error: true,
    ...
    if (!result) {
        result = app.getModel('PaymentProcessor').handle(cart.object, selectedPaymentMethodID);
    }
    return result;
}
    ...
    app.getForm('billing').handleAction({
        ...
        save: function () {
            Transaction.wrap(function () {
                ...
                var handleBillingAddressResult = handleBillingAddress(cart);
                var handlePaymentSelectionResult = handlePaymentSelection(cart);
	
                if (!resetPaymentFormsResult || !validateBillingResult || !handleBillingAddressResult || // Performs validation steps, based upon the entered billing address
                // and address options.
