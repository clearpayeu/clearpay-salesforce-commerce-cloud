...
var clearpay = {
    redirect: function () {
        ...
        var isClearpayUrl = $('input[name="dwfrm_clearpay_isClearpayUrl"]').val();

        $.ajax({
            url: isClearpayUrl + '?csrf_token=' + csrfToken,
            type: 'post',
            success: function (data) {
                if (typeof (data && data.isClearpay) !== 'undefined' && data.isClearpay) {
                    // payment method is clearpay
                    $('button.submit-payment').text(data.resource.pleaseWait);
                    var redirectMessage = data.resource.redirectMessage;
                    var redirectClearpayUrl = $('input[name="dwfrm_clearpay_redirectClearpayUrl"]').val();
                    // get redirect url
                    $.ajax({
                        url: redirectClearpayUrl + '?csrf_token=' + csrfToken,
                        type: 'post',
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.error) {
                    ...
                    if (data.clearpayEnabled) {
                        // Initalize clearpayCheckout
                        clearpay.redirect();
                    } else {
                        defer.resolve(data);
                    }
