 ...
 * @param {Object} res - response
 * @param {Object} next - next
 * @param {Object} clearPayOrderResponse - Clearpay Order Response
 * @returns {Object} - next
 */
...
function integratedShippingFlow(req, res, next, clearPayOrderResponse) {
    ...
    var cartTotals = ClearpayShippingHelpers.calculateBasketTaxShipTotals(req, currentBasket);
    ClearpayCOHelpers.addConsumerToBasket(currentBasket, clearPayOrderResponse.consumer);
    ...
    if (buyNow) {
        ...
        require('*/cartridge/scripts/checkout/clearpayUpdatePreapprovalStatus').getPreApprovalResult(currentBasket, {
            status: 'SUCCESS', orderToken: cpOrderToken, cpExpressCheckout: true
        });
...
server.get('Debug', server.middleware.https, function (req, res, next) {
    var currentBasket = BasketMgr.getCurrentBasket();
    ClearpayRefArchCOHelpers.removeAllNonGiftCertificatePayments(currentBasket);
    ClearpayShippingHelpers.calculateBasketTaxShipTotals(req, currentBasket);
    return next();
});
...
server.post('GetShippingMethods', server.middleware.https, function (req, res, next) {
    ...
    // if this is a store pickup, just get the store name
        ...
        // The cart should only have in-store pickup items at this point
            var costs = ClearpayShippingHelpers.calculateBasketTaxShipTotals(req, currentBasket);
            res.json({
                shipmethods: [{
                    id: store.ID,
                    name: store.name,
...
server.get('PostClearpayCheckoutFlow', server.middleware.https, function (req, res, next) {
    if (!clearpayEnabled) {
