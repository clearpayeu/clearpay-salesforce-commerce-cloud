...
var Logger = LogUtils.getLogger('clearpayUtils');
...
*/
clearpayUtils.filterLogData = function (requestJSON) {
    if (requestJSON) {
        var request = JSON.parse(requestJSON);
        var filteredResponse;

        if (request.orderDetails) {
            filteredResponse = clearpayUtils.maskDetails(request.orderDetails);
        } else {
            filteredResponse = clearpayUtils.maskDetails(request);
        }
...
* @returns {string} - request JSON
*/
clearpayUtils.maskDetails = function (requestJSON) {
    var index;
...
function getErrorResponse(errorCode) {
    ...
    var defaultMessage = Resource.msg('apierror.flow.default', 'clearpay', null);
    var errorMessage = Resource.msg(errorResource, 'clearpay', defaultMessage);
...
function logResponseData(urlPath, httpVerb, requestBody, result, errorWasThrown) {
    try {
        ...
        requestBodyJson = clearpayUtils.filterLogData(requestBodyJson);
        var responseBodyJson = result.object;
        responseBodyJson = clearpayUtils.filterLogData(responseBodyJson);
...
/**
* checks whether clearpay is enabled or not in the configuration and checks the minimum and maximum threshold
* @param {string} urlPath - url Path
* @param {string} httpVerb - httpVerb
* @param {Object} result - result of the response
* @param {string} requestBody - request Body
...
clearpayUtils.handleServiceResponses = function (urlPath, httpVerb, result, requestBody, errorDelegate) {
    if (result.status !== 'OK') {
        Logger.error('Error on Clearpay Service execution: ' + result.errorMessage);
    }
