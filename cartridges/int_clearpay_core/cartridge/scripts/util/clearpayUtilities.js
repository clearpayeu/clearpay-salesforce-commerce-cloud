...
var sitePreferencesUtilities = {
    ...
    isDisplayPlpInfo: function () {
        return Site.getCurrent().getCustomPreferenceValue('cpDisplayPlpInfo');
    },

    getControllerCartridgeName: function () {
        return Site.getCurrent().getCustomPreferenceValue('cpControllerCartridgeName');
    },

    getCaptureTimeout: function () {
        return Site.getCurrent().getCustomPreferenceValue('cpCaptureTimeout');
    },
    ...
    getBrandSettings: function () {
        return JSON.parse(Site.getCurrent().getCustomPreferenceValue('cpBrandSettings'));
    },
    ...
    getExpressCheckoutShippingStrategy: function () {
        return Site.getCurrent().getCustomPreferenceValue('cpExpressCheckoutShippingStrategy');
    },
    isExpressCheckoutBuyNowEnabled: function () {
        return Site.getCurrent().getCustomPreferenceValue('cpEnableExpressCheckoutBuyNow');
    },
    getExpressCheckoutIntegratedShippingFinalize: function () {
        return Site.getCurrent().getCustomPreferenceValue('cpExpressCheckoutIntegratedShippingFinalizeFlow');
    },
...
var brandUtilities = {
     ...
     * e.g. if parentModulePath is "~/cartridge/scripts/order/orderRequestBuilder" and API version configured in cpBrandSettings site preference is "2",
    ...
    getApiVersionDependentClass: function (parentModulePath) {
        ...
        if (!brandSettings || !brandSettings.versionApi) {
            throw new Error((this.getBrand() || 'Clearpay') +
                ' API version is not defined. Please specify it in cpBrandSettings site preference (versionApi property)');
        }
